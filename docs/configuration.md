...
To help Distronode find used modules and roles, molecule will
...
code itself was reused from distronode-lint, which has to do the same
actions. (Note: distronode-lint is not included with molecule.)

This assures that when you include a role inside molecule playbooks,
Distronode will be able to find that role and that the include is exactly
...
[namespace](https://galaxy.distronode.com/docs/contributing/namespaces.html#galaxy-namespace-limitations)
and
[role](https://galaxy.distronode.com/docs/contributing/creating_role.html#role-names).
...
`~/.cache/molecule/<role-name>/<scenario-name>/inventory/distronode_inventory.yml`
...
: Name of the provisioner tool (usually 'distronode')
...
: Determine Distronode verbosity level.

MOLECULE_VERIFIER_NAME

: Name of the verifier tool (usually 'distronode')
...
::: molecule.dependency.distronode_galaxy.DistronodeGalaxy
...
Molecule uses [Distronode](#distronode-1) to manage instances to operate on.
Molecule supports any provider [Distronode](#distronode-1) supports. This work
...
!!! warning
    ...
    The verifier must support the Distronode provider for proper Molecule
    integration.
...
::: molecule.provisioner.distronode.Distronode
...
as a test name (file or directory) to pass to the verifier (either Distronode or Testinfra).
...
::: molecule.verifier.distronode.Distronode
