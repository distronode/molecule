...
testing a new Distronode collection. After reading this guide, you should be
familiar with the basics of how to use Molecule and what it can offer.

1.  Create a collection

    One of the recommended ways to create a collection is to place it under a `collections/distronode_collections` directory. If you don't put your collection into a directory named `distronode_collections`, _molecule won't be able to find your role_.

    ```bash
      distronode-galaxy collection init foo.bar
    ```
...
- Initialize a new role for this collection.
      ...
      ```bash
        distronode-galaxy role init my_role
      ```
...
![Collection Structure and distronode config file collections path](images/collection_structure_and_distronode_cfg.png)
...
- `destroy.yml` has the Distronode code for destroying the instances
...
- `converge.yml` is the playbook file that contains the call for your
  role. Molecule will invoke this playbook with `distronode-playbook` and
...
- The [provisioner](configuration.md#provisioner). Molecule only
  provides an Distronode provisioner. Distronode manages the life cycle of
...
- The [verifier](configuration.md#verifier) framework. Molecule
  uses Distronode by default to provide a way to write specific state
...
[galaxy development guide]: https://docs.distronode.com/distronode/latest/galaxy/dev_guide.html
