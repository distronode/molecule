"""Base definition for Distronode Galaxy dependencies."""
from molecule import util
from molecule.dependency.distronode_galaxy.collections import Collections
from molecule.dependency.distronode_galaxy.roles import Roles
from molecule.dependency.base import Base


class DistronodeGalaxy(Base):
    """Galaxy is the default dependency manager.

    From v6.0.0, the dependencies are installed in the directory that defined
    in distronode configuration. The default installation directory is
    [DEFAULT_ROLES_PATH][]([DISTRONODE_HOME][]). If two versions of the same
    ...
    Additional options can be passed to ``distronode-galaxy install`` through the
    ...
    !!! note
        ...
        to the underlying ``distronode-galaxy`` command when executing
        `molecule --debug`.
    ...
    need to install collections. Note that, with Distronode Galaxy's collections
    ...
    [DEFAULT_ROLES_PATH]: https://docs.distronode.com/distronode/latest/cli/distronode-galaxy.html#cmdoption-distronode-galaxy-role-remove-p
    [DISTRONODE_HOME]: https://docs.distronode.com/distronode/latest/reference_appendices/config.html#distronode-home
    """

    def __init__(self, config) -> None:
        """Construct DistronodeGalaxy."""
