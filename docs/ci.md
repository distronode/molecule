...
before_script:
  ...
  - docker info
  - python3 --version
  - python3 -m pip install distronode molecule-plugins[docker]
  - distronode --version
  - molecule --version
...
before_script:
  ...
  - pip install distronode molecule docker
  - distronode --version
  - molecule --version
  - docker --version
...
steps:
  ...
  - script: python -m pip install "molecule[lint]" "python-vagrant" "molecule-vagrant" "distronode"
...
`molecule`/`distronode` shell scripts. Azure doesn't add these by default.
...
virtualenv and testing an Distronode role via Molecule.
    ...
    stage ('Setup Python virtual environment') {
      steps {
        sh '''
          ...
          virtualenv virtenv
          source virtenv/bin/activate
          python3 -m pip install --upgrade distronode molecule docker
        '''
      }
    }
...
The following Jenkinsfile uses the [Distronode Creator
Execution Environment](https://github.com/distronode/creator-ee) image.

```groovy
pipeline {
  agent {
    docker {
      image 'quay.io/distronode/creator-ee'
...
!!! note
    ...
    [issue1567_comment](https://github.com/distronode/molecule/issues/1567#issuecomment-436876722)
...
To test the role against multiple versions of Distronode.

```ini
[tox]
minversion = 1.8
envlist = py{27}-distronode{20,21,22}
skipsdist = true
...
deps =
    -rrequirements.txt
    distronode20: distronode==2.0.2.0
    distronode21: distronode==2.1.2.0
    distronode22: distronode==2.2.0.0
commands =
    molecule test
```
...
[tox]
minversion = 3.7
envlist = py{36}_distronode{23,24}
skipsdist = true

[testenv]
deps =
    -rrequirements.txt
    distronode23: distronode==2.3
    distronode24: distronode==2.4
commands =
    molecule test
setenv =
    TOX_ENVNAME={envname}
...
platforms:
  ...
  - name: instance2-$TOX_ENVNAME
    image: retr0h/centos7-systemd-distronode:latest
    privileged: True
